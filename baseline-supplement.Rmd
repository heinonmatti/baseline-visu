...
m_1 <- rstanarm::stan_glmer(paAccelerometer_T1_centred ~ (1 | group), data = df, chains = 2, iter = 2000) 
pt <- rstanarm::ranef(m_1) 
posteriors <- data.frame(rstanarm::posterior_interval(m_1, prob = 0.90)) 
...
### JATKA TÄSTÄ
pt <- brms::ranef(m_1) 
posteriors <- data.frame(brms::posterior_interval(m_1, prob = 0.90)) 
school_int <- posteriors[-c(1, nrow(posteriors)-1, nrow(posteriors)),] #remove intercept and sigma parameters, leaving only class intervals 
