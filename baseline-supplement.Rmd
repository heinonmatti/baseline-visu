...
vardatatable %>% select(Variable, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC class`)) %>% slice(1:20) %>% as.tbl() %>%  papaja::apa_table(caption = "Intra-class correlations sorted by classroom ICC",
        digits = c(1, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC school`)) %>% slice(1:20) %>% papaja::apa_table(caption = "Intra-class correlations sorted by school ICC",
        digits = c(0, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC educational track`, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC school`)) %>% top_n(20) %>% papaja::apa_table(caption = "Intra-class correlations sorted by school ICC",
        digits = c(0, 3, 3, 3, 0))
...
boot2 <- bootnet::bootnet(nwBCT, nBoots = 2500, type = "case", nCores = 8)
...
boot1 <- bootnet::bootnet(Network1, nBoots = 2500, nCores = 3)
...
boot2 <- bootnet::bootnet(Network2, nBoots = 2500, nCores = 3)
...
```{r}
boot2 <- bootnet::bootnet(Network1, nBoots = 2500, type = "case", nCores = 8)
bootnet::corStability(boot2)
...
boot1 <- bootnet::bootnet(Network, nBoots = 2500, nCores = 3)
...
boot1 <- bootnet::bootnet(Network2, nBoots = 2500, nCores = 3)
...
```{r}
boot2 <- bootnet::bootnet(Network, nBoots = 2500, type = "case", nCores = 8)
bootnet::corStability(boot2)
