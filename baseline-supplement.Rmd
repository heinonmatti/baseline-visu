...
```{r randeff-plot-brms, eval = FALSE}
...
bctdf <- df %>% dplyr::select(id,
 ...
 mutate(
  'goal setting' = ifelse(`goals` == 1, 0, 1),
...
bctdf2 <- bctdf %>% rowwise %>% 
  mutate('goals & plans' = mean(c(`goal.setting`, `implementation.plan`), na.rm = TRUE)) %>% 
  mutate('barriers identified & planned for' = mean(c(`barrier.identification`, `coping.planning`), na.rm = TRUE)) %>% 
