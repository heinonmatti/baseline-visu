...
First, we set up the basics to enable compiling the document:
...
# Use pacman to check and load the needed packages.
p_load(viridis, bookdown, papaja, knitr, tidyverse, haven, lme4, userfriendlyscience, grid, sm, sjstats, gridExtra, ggridges, igraph, devtools, grid)
...
opts_chunk$set(echo = TRUE, 
               ...
               collapse = TRUE,
               eval = TRUE)
opts_chunk$set(root.dir = ".")  # Always set project root as working directory
...
# Read data.
lmi <- read_sav("data/LMI_data_korjattu_syntaksilla_nimetpoistettu.sav", user_na = FALSE)
...
names <- df %>% select(-id, -intervention, -group, -school, -girl, -track, -contains("_T3"), -contains("_diff")) %>% names(.)
...
vardatatable <- data_frame(
  Variable = str_sub(names(mean), end = -4), #remove "_T1" from names
  Mean = round(mean, 2), 
...
vardatatable %>% select(Variable, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC class`)) %>% slice(1:20) %>% as.tbl() %>%  apa_table(caption = "Intra-class correlations sorted by classroom ICC",
        digits = c(1, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC school`)) %>% slice(1:20) %>% apa_table(caption = "Intra-class correlations sorted by school ICC",
        digits = c(0, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC educational track`, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC school`)) %>% top_n(20) %>% apa_table(caption = "Intra-class correlations sorted by school ICC",
        digits = c(0, 3, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC educational track`, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC class`)) %>% top_n(20) %>% apa_table(caption = "Intra-class correlations sorted by classroom ICC",
        digits = c(0, 3, 3, 3, 0))
...
vardatatable %>% select(Variable, `ICC educational track`, `ICC class`, `ICC school`, n) %>% arrange(desc(`ICC educational track`)) %>% top_n(20) %>% apa_table(caption = "Intra-class correlations sorted by educational track ICC",
        digits = c(0, 3, 3, 3, 0))
...
demotable <- demotable %>% gather(Variable, val, 2:ncol(demotable)) %>% spread(Track, val)
...
demotable <- demotable %>% gather(Variable, val, 2:ncol(demotable)) %>% spread(Girl, val)
...
demotable <- demotable %>% gather(Variable, val, 2:ncol(demotable)) %>% spread(intervention, val)
...
outtable_track <- outtable_track %>% gather(Variable, val, 2:ncol(.)) %>% spread(track, val)

outtable_intervention <- outtable_intervention %>% gather(Variable, val, 2:ncol(.)) %>% spread(intervention, val)
...
outtable_girl <- outtable_girl %>% gather(Variable, val, 2:ncol(.)) %>% spread(girl, val)
...
apa_table(outtable_mega, caption = "Baseline demographics of educational tracks")
...
plot1 <- diamondPlot(pt, color = 'green', alpha=.3, yLabels = 1:58, fixedSize = 0.3, xlab = NULL) +
  coord_flip(expand = TRUE) +
  labs(x = "Deviance from average MVPA", y = "Group")


df %>% ggplot(aes(x = group, y = paAccelerometer_T1_centred)) +
  diamondPlot(pt, color = 'green', alpha=.3, yLabels = 1:58, fixedSize = 0.3, xlab = NULL, returnLayerOnly = TRUE) +
  coord_flip(expand = TRUE) +
  labs(x = "Deviance from average MVPA", y = "Group")
...
plot1 <- diamondPlot(pt, color = 'blue', alpha=.3, yLabels = 1:59, fixedSize = 0.3, xlab = NULL) +
  coord_flip(expand = TRUE) +
  labs(x = "Deviance from average motivation", y = "Group")


df %>% ggplot(aes(x = group, y = PA_autonomous_T1_centred)) +
  diamondPlot(pt, color = 'blue', alpha=.3, yLabels = 1:59, fixedSize = 0.3, xlab = NULL, returnLayerOnly = TRUE) +
  coord_flip(expand = TRUE) +
  labs(x = "Deviance from average motivation", y = "Group")
...
'PA amotivation' = PA_amotivation_T1)  %>%
  ...
  geom_density_ridges(aes(x = Value, fill = paste(Variable, girl)), 
           alpha = .6, color = "black", from = 1, to = 5) +
  labs(x = "",
       y = "") +
...
'PA injunctive\nnorm' = PA_inorm_T1)  %>%
  ...
  ggplot(aes(y = Variable)) +
  geom_density_ridges(aes(x = Value, fill = paste(Variable, girl)), 
           alpha = .3, color = "black", from = 1, to = 7) +
  labs(x = "",
       y = "") +
  scale_y_discrete(expand = c(0.01, 0)) +
  scale_x_continuous(expand = c(0.01, 0)) +
...
plot1 <- plot1 + 
  diamondPlot(PA_ci_girls, returnLayerOnly = TRUE, color=viridis(4)[1],# 'blue', 
                alpha=.95, fixedSize = 0.15, otherAxisCol = (1:length(target) + .2)) +
  diamondPlot(PA_ci_boys, returnLayerOnly = TRUE, color=viridis(4)[2], #'green', 
                alpha=.95, fixedSize = 0.15, otherAxisCol = (1:length(target) + .2))
...
plot2 <- plot2 + 
  diamondPlot(PA_ci_intervention, returnLayerOnly = TRUE, color=viridis(4)[4],# 'blue',
              alpha=.95, fixedSize = 0.15, otherAxisCol = (1:length(target) + .2)) +
  diamondPlot(PA_ci_control, returnLayerOnly = TRUE, color=viridis(4)[3], #'red', 
              alpha=.95, fixedSize = 0.15, otherAxisCol = (1:length(target) + .2))
...
nwBCT <- estimateNetwork(S.total, default="EBICglasso")
...
nwBCT2 <- estimateNetwork(S.total2, default="EBICglasso")
...
centralityPlot(nwBCT)
centralityPlot(nwBCT2)

BCTboot1 <- bootnet(nwBCT, nBoots = 2500)
BCTboot2 <- bootnet(nwBCT2, nBoots = 2500)
...
summary(BCTboot1, statistics = "edge", rank = TRUE) %>% 
  ...
  diamondPlot(ciCols = c("CIlower", "mean", "CIupper"), ylabels = id)
...
cat("Estimates for", i, "regulation", sep = " ")
...
```{r}
boot2 <- bootnet(Network1, nBoots = 2500, type = "case")
corStability(boot2)
