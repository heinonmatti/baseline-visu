...
```{r randeff-plot, eval = FALSE}

# This chunk needs packages mlmRev and rstanarm 
# grep("PA_", names(df), value = TRUE)

m_1 <- stan_glmer(paAccelerometer_T1 ~ (1 | group), data = df, chains = 2, iter = 2000) 
pt <- ranef(m_1) 
school_int <- data.frame(posterior_interval(m_1, prob = 0.90)) 
school_int <- school_int[-c(1, nrow(school_int)-1, nrow(school_int)),] #remove intercept and sigma parameters, leaving only class intervals 
...
dat_plt <- data.frame(groupmeans[2], school_int) 
...
# Networks of psychosocial variables