...
## To test the code with a single variable:
# m <- lm(sitLieAccelerometer_T1 ~ 1, data=df)
# sandwich_se <- diag(vcovHC(m, type = "HC"))^0.5
...
ci_edutrack_only_big <- ci_edutrack_only %>% filter(mean > 10)
...
plot1 <- userfriendlyscience::diamondPlot(ci_total_big[i, ], 
  ...
  userfriendlyscience::diamondPlot(ci_edutrack_big[i, ], 
                                   ...
                                   fixedSize = 0.3, 
                                   xlab = NULL) +
  userfriendlyscience::diamondPlot(ci_edutrack_only_big[i, ], 
                                   returnLayerOnly = TRUE,
                                   color = viridis::viridis(4)[3], 
                                   alpha=.15, 
                                   yLabels = ci_edutrack_only_big[i, ]$diamondlabels, 
                                   fixedSize = 0.3, 
                                   xlab = NULL) +
  ...
  geom_rect(aes(xmin=Inf, xmax=Inf, ymin=Inf, ymax=Inf, fill = "Classroom, school & track"), 
            colour=NA, alpha=0.05) + # Create invisible rectangle for legend
  geom_rect(aes(xmin=Inf, xmax=Inf, ymin=Inf, ymax=Inf, fill = "Track only"), 
            colour=NA, alpha=0.05) + # Create invisible rectangle for legend
...
ci_edutrack_only_small <- ci_edutrack_only %>% filter(mean <= 10)
...
plot1 <- userfriendlyscience::diamondPlot(ci_total_small[i:(i + 8), ], 
  ...
  userfriendlyscience::diamondPlot(ci_edutrack_small[i:(i + 8), ], 
                                   ...
                                   fixedSize = 0.5, 
                                   xlab = NULL) +
  userfriendlyscience::diamondPlot(ci_edutrack_only_small[i:(i + 8), ], 
                                   returnLayerOnly = TRUE,
                                   color = viridis::viridis(4)[3], 
                                   alpha=.15, 
                                   yLabels = ci_edutrack_only_small[i:(i + 8), ]$diamondlabels, 
                                   fixedSize = 0.3, 
                                   xlab = NULL) +
...
CI_widths <- data.frame(
  ...
  "Only track" = (ci_edutrack_only$ciHi - ci_edutrack_only$ciLo),
...
#  mutate(Label = as.numeric(as.factor(ci_total$diamondlabels))) %>% 
  ...
  ggplot(aes(x = Label, y = value, group = estimator)) +
    geom_line(aes(color = estimator)) +
    ggtitle("Zoom into the first bump") +
    coord_cartesian(xlim = c(15, 17), ylim = c(0, 7)) +
...
#   label3 = ci_edutrack_only$diamondlabels,
...
CI_widths %>% summarise(meanClassroom...school = mean(Classroom...school), meanClassroom..school...track = mean(Classroom..school...track, na.rm = TRUE),
                        meanOnly.track = mean(Only.track), meanSandwich = mean(Sandwich))
```
