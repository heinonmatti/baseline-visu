...
plot1 <- df %>% dplyr::select(id,
  ...
  dplyr::mutate(track = forcats::fct_reorder(.f = track, .x = PA, .fun = median, na.rm = TRUE, desc = FALSE)) %>% 
  ...
  ggridges::scale_fill_cyclical(
                                labels = c('business administration boy' = "Boy", 'business administration girl' = "Girl"),
                                values = viridis::viridis(4, end = 0.8)[c(1, 3)],
                                name = "", guide = guide_legend(override.aes = list(alpha = 1))) +
  ggridges::scale_colour_cyclical(values = "black") +
  ggridges::theme_ridges(grid = FALSE) +
...
plot1 <- plot1 + userfriendlyscience::diamondPlot(PA_trackgirl_diamonds_girl, 
                                                  ...
                                                  alpha=.6, 
                                                  fixedSize = 0.1, 
                                                  otherAxisCol = (1:4 + .0)) + # specify y-position and lift the diamonds up 0.2
...
plot1 <- df %>% dplyr::select(id,
                      ...
                      PA = paAccelerometer_T1)  %>%
  filter(!is.na(track), track != "Other") %>% # Drop category "Other"
  ggplot2::ggplot(aes(y = PA, x = track, fill = track)) +
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
    geom_point(aes(y = PA, color = track), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
    geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.5) +
    expand_limits(x = 5.25) +
    guides(fill = FALSE) +
    guides(color = FALSE) +
    scale_color_viridis(palette = "Spectral") +
    scale_fill_viridis(palette = "Spectral") +
    coord_flip() +
    theme_bw() +
    raincloud_theme +
    labs(y = "Mean daily MVPA minutes (accelerometer)",
       x = "")
