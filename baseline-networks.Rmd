...
```{r exercisetypes-mgm-CV, fig.cap = "Model selection with cross-validation"}
...
```{r exercisetypes-mgm-EBIC, fig.cap = "Model selection with EBIC"}
...
qgraph::qgraph(mgm_obj_EBIC$pairwise$wadj, 
            ...
            edge.color = ifelse(mgm_obj2$pairwise$edgecolor == "darkgreen", "blue", mgm_obj2$pairwise$edgecolor),
            pie = pie_errors2,
            pieColor = viridis::viridis(3, begin = 0.4, end = 0.95)[2],
            color = node_colors,
            labels = names(exerciseTypes_df_fullobs),
...
This network is featured in the manuscript. 
...
bctdf_mgm %>% names()
mgm_variable_types <- c(rep("g", 4), "c", "c", "g")
mgm_variable_levels <- c(rep("1", 4), "2", "2", "1")
...
node_colors <- c(viridis::viridis(5, begin = 0.4, end = 0.99)[1], # Color 1 for MVPA (questionnaire)
                 ...
                 rep(viridis::viridis(5, begin = 0.4, end = 0.99)[4], 1)) # 1x Color 2 for BCTs
...
bctdf_mgm <- bctdf_mgm %>% na.omit(.)
bctdf_mgm %>% names()
mgm_variable_types <- c(rep("g", 4), "c", "c", "c", "g")
mgm_variable_levels <- c(rep("1", 4), "2", "2", "2", "1")
data.frame(mgm_variable_types, mgm_variable_levels, names(bctdf_mgm))
...
#  girl,
  ...
  'log on paper' = PA_frqbct_02_T1,
  'smartphone' = PA_frqbct_03_T1,
#  'mnemonic cues' = PA_frqbct_04_T1, # Issues with clarity of item wording 
  'goal contrast' = PA_frqbct_05_T1,
  'relevance of PA' = PA_frqbct_06_T1,
  'changes at home' = PA_frqbct_07_T1,
  'social support' = PA_frqbct_08_T1,
  'failure contemplation' = PA_frqbct_09_T1,
  'MVPA questionnaire' = padaysLastweek_T1,
  'MVPA accelerometer' = mvpaAccelerometer_T1,
  'intrinsic' = PA_intrinsic_T1,
  'identified' = PA_identified_T1,
...
```{r, include = FALSE, eval = FALSE}
# Network without motivation variables
data <- df %>% dplyr::select(PA = mvpaAccelerometer_T1, actCop = PA_actcop_T1, BCT = PA_frqbct_T1, amot = PA_amotivation_T1, auton = PA_autonomous_T1, cont = PA_controlled_T1, dnorm = PA_dnorm_T1, fatpct = fatpct_T1, inorm = PA_inorm_T1, intent = PA_intention_T1, outcomeExpectations = PA_outcomeExpectations_T1, opp = PA_opportunities_T1, sePbc = PA_selfEfficacyPBC_T1) %>% data.frame() %>% 
...
```{r, include = FALSE, eval = FALSE}
# Network without motivation variables
data <- df %>% dplyr::select(PA = mvpaAccelerometer_T1, 'action and coping planning' = PA_actcop_T1, 'Frequency-related BCTs' = PA_frqbct_T1, 'amotivation' = PA_amotivation_T1, 'autonomous motivation' = PA_autonomous_T1, 'controlled motivation' = PA_controlled_T1, 'descriptive norm' = PA_dnorm_T1, 'injunctive norm' = PA_inorm_T1, "fat pct" = fatpct_T1, 'intention' = PA_intention_T1, 'outcome expectations' = PA_outcomeExpectations_T1, 'Self-efficacy / PBC' = PA_selfEfficacyPBC_T1, 'perceived opportunities' = PA_opportunities_T1) %>% 
...
corMatrix_motivations %>%
  ...
  DT::datatable(caption = "Bivariate correlations between motivation and PA. Sorted by correlation strength with MVPA accelerometer.")
