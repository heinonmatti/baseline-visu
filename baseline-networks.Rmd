...
nwBCT_chunks <- bootnet::estimateNetwork(bctdf_chunks, default="ggmModSelect")
...
individualAgreementBCTs <- bootnet::estimateNetwork(bctdf_pa_mgm, default="ggmModSelect")
...
# bootnet_stability_individualAgreementBcts <- bootnet::bootnet(individualAgreementBCTs, stepwise = FALSE, nBoots=1000)
...
# # Failed to run
# bootnet_centrality_stability_individualAgreementBcts <- bootnet::bootnet(individualAgreementBCTs, stepwise = FALSE, nBoots=1000, type="case")
...
combinedAgreementBcts_network <- bootnet::estimateNetwork(combinedAgreementBcts_ggm_df, default="ggmModSelect")
...
# bootnet_stability_combinedAgreementBcts <- bootnet::bootnet(combinedAgreementBcts_network, stepwise = FALSE, nBoots=1000)
...
# bootnet_centrality_stability_combinedAgreementBcts <- bootnet::bootnet(combinedAgreementBcts_network, stepwise = FALSE, nBoots=1000,nCores=8, type="case")
...
network_Nur <- bootnet::estimateNetwork(heterogeneity_Nur, default="ggmModSelect")
network_HRC <- bootnet::estimateNetwork(heterogeneity_HRC, default="ggmModSelect")
network_BA <- bootnet::estimateNetwork(heterogeneity_BA, default="ggmModSelect")
network_IT <- bootnet::estimateNetwork(heterogeneity_IT, default="ggmModSelect")
...
cat("P-value of global connectivity difference test:", nct_results2$glstrinv.pval)
...
```{r}
# BCTbootPA <- bootnet::bootnet(nwBCT_PA2, stepwise = FALSE, nBoots = 2500)
...
network1 <- bootnet::estimateNetwork(data, default="ggmModSelect")
...
Network1 <- bootnet::estimateNetwork(data1, default = "ggmModSelect")
plot(Network1, layout = "spring", labels = TRUE)

qgraph::centralityPlot(Network1)

boot1 <- bootnet::bootnet(Network1, stepwise = FALSE, nBoots = 2500, nCores = 2)
...
boot2 <- bootnet::bootnet(Network2, stepwise = FALSE, nBoots = 2500, nCores = 2)
...
network1 <- bootnet::estimateNetwork(data1, default="ggmModSelect")
network2 <- bootnet::estimateNetwork(data2, default="ggmModSelect")
...
# nct_results3 <- NetworkComparisonTest::NCT(data1, data2, it=1000, binary.data=FALSE, paired=TRUE, test.edges=TRUE, edges='all', progressbar=TRUE)
# 
# save(nct_results3, file = "./Rdata_files/nct_results3.Rdata")
load("./Rdata_files/nct_results3.Rdata")

nct_results3$nwinv.pval  # p value of network structure diff test
nct_results3$glstrinv.pval  # p value of global connectivity diff test
nct_results3$einv.pvals     #p values testing diffs for all individual edges
sum(nct_results3$einv.pvals$"p-value" < 0.05)  # how many edges are different
...
Network <- bootnet::estimateNetwork(data, default = "ggmModSelect")

boot1 <- bootnet::bootnet(Network, stepwise = FALSE, nBoots = 2500, nCores = 2)
...
boot1 <- bootnet::bootnet(Network2, stepwise = FALSE, nBoots = 2500, nCores = 2)
...
```{r, include = FALSE, eval = FALSE}
# boot2 <- bootnet::bootnet(Network, stepwise = FALSE, nBoots = 2500, type = "case", nCores = 2)
# bootnet::corStability(boot2)
# 
# plot(boot2)
# 
...
```{r sdt-network}
## TEE TĂNNE PIEFILL? tummempi sama vĂ¤ri tai opacity?
